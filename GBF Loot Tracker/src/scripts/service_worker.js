import '../scripts/types/typedefs.js';
...
class DataProcessor {
    ...
    static ProcessGuildWarLanding(bodyDataString){
        ...
        const honorSumLocation = `Total</div><div class="txt-total-record"><em>`
        const honorDailyLocation = `Daily</div><div class="txt-total-record"><em>`
    ...
    static FindEnemyName(lootList, resultData) {
        ...
        switch (quest_type) {
            ...
            case 25:
                ...
                if (urlFilter.sandboxStavesRegex.test(url)) { console.log(url + " is in zone E-H"); return sephiraData.zones[url]; }
                // Checks if stage is in Sephira Zones I-L
                if (urlFilter.sandboxSwordsRegex.test(url)) {
    ...
    static FindEventEnemyName(lootList, resultData) {
        ...
        if (+eventType == 3 && tokenWinning == 30 && tokenSum == 30){ // Special case where the guild war boss could be either hard or nm95...
            // WTF Cygames... They made hard and nm95 have the same token victory reward but balanced it by not giving hard an mvp or host reward...
            // Because of this, I won't track nm95 fights that the user didn't host or mvp. I hope this doesn't affect many users...
            return "Unknown"
        }
